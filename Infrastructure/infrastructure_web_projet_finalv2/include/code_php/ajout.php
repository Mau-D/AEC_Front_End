...
// Vérification que l'action d'ajout a été soumise et que tous les champs sont présents
  if(isset($_POST['ajoutSubmit']) && isset($_POST['titre']) && isset($_POST['description_courte']) && isset($_POST['description_longue']) && isset($_POST['date_nouvelle']) && isset($_POST['actif']) && isset($_POST['categorieID'])) { 
    ...
    if ($requete = $mysqli->prepare("INSERT INTO nouvelles(titre, description_courte, description_longue, date_nouvelle, actif, fk_categorie) VALUES(?, ?, ?, ?, ?, ?)")) {  // Création d'une requête préparée 
      ...
      /**************************************************************/
      $requete->bind_param("ssssii", $_POST['titre'], $_POST['description_courte'], $_POST['description_longue'], $_POST['date_nouvelle'], $_POST['actif'], $_POST['categorieID']); // Envoi des paramètres à la requête. 

      if($requete->execute()) { // Exécution de la requête
        $messageAjout = "<div class='alert alert-success text-center'>Nouvelle ajoutée!</div>";  // Message ajouté dans la page en cas d'ajout réussi
      } else {
